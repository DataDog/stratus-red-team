...
func (m *FileSystemStateManager) GetTerraformVariables() (map[string]string, error) {
	variablesPath := m.getVariablesStateFile()
	variables := make(map[string]string)

	// If we have persisted Terraform variables on disk, read them
	if m.FileSystem.FileExists(variablesPath) {
		variablesString, err := m.FileSystem.ReadFile(variablesPath)
		if err != nil {
			return nil, err
		}

		err = json.Unmarshal(variablesString, &variables)
		if err != nil {
			return nil, err
		}
	}
...
func (m *FileSystemStateManager) getVariablesStateFile() string {
	return filepath.Join(m.RootDirectory, m.Technique.ID, StratusStateTerraformVariablesFileName)
}
